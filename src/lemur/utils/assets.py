...
def get_dir_contents(path: str) -> dict[str, list[str]]:
    ...
    directory_path = _get_safe_path(base_path, target_suffix, is_dir=True)
    return {
...
def _get_file_contents(path: Path, suffix_path: str) -> str:
    with open(_get_safe_path(path, suffix_path), "r", encoding="utf-8") as file:
        return file.read()

def _get_safe_path(base_path: Path, suffix_path: str, is_dir: bool = False) -> Path:
